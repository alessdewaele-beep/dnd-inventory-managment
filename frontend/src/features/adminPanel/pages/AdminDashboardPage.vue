...
// Role -> count, as a readable string to show under the users card.
...
<template>
  <div>
    ...
    <p class="text-sm opacity-70 mb-6">Overview of the application.</p>
    ...
    <div v-if="stats" class="flex flex-col gap-6">
        ...
        <div class="rounded-xl border border-gold/50 bg-panel dark:bg-panel-dark p-5 shadow-sm">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-sm opacity-70">Campaigns</div>
            </div>
          </div>
          <div class="mt-3 text-xs opacity-70">
            {{ stats.campaigns.withDm }} with a DM assigned
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="rounded-xl border border-gold/50 bg-panel dark:bg-panel-dark p-5 shadow-sm">
          <h3 class="font-serif text-lg mb-3">Newest users</h3>
        ...
        <div class="rounded-xl border border-gold/50 bg-panel dark:bg-panel-dark p-5 shadow-sm">
          <h3 class="font-serif text-lg mb-3">Newest items</h3>
          <ul class="divide-y divide-gold/20">
            <li
            ...
            >
              <span class="flex items-center gap-2">
                ...
                <span class="opacity-60">· {{ it.owner || "unknown" }}</span>
              </span>
    ...
    <div v-else-if="adminService.state.loading" class="opacity-70">Loading…</div>
  </div>
</template>
